...
export const authGeneralSettingsSchema = z.object({
  disable_signup: z
    .boolean()
    .optional()
    .describe('This will prevent new users from signing up.'),
...
export const authPhoneProviderSchema = z
  .object({
    ...
    sms_template: z
      .string()
      .optional()
      .describe('To format the OTP code use `{{ .Code }}`'),
    sms_test_otp: z
      .string()
      .optional()
      .describe(
...
export const authGoogleProviderObject = z.object({
  external_google_enabled: z
    .boolean()
    .optional()
    .describe('Enable Sign in with Google'),
...
export const authGoogleProviderSchema = authGoogleProviderObject
  .superRefine((data, ctx) => {
    ...
    if (
      data.external_google_client_id &&
      data.external_google_client_id.includes(' ')
    ) {
      ctx.addIssue({
        code: 'custom',
        path: ['external_google_client_id'],
